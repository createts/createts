 ...
 * Rewritten from https://github.com/mapbox/earcut/blob/master/src/earcut.js.
 */
...
export function earcut(data: number[], holeIndices?: number[], dim?: number): number[] {
  dim = dim || 2;
  ...
  if (data.length > 80 * dim) {
    ...
    // The minX, minY and invSize are later used to transform coords into integers for z-order
    // calculation.
  ...
  do {
    ...
    while (b !== a.prev) {
      if (a.i !== b.i && isValidDiagonal(a, b)) {
        ...
        // Filter colinear points around the cut.
...
// Link every hole into the outer loop, producing a single-ring polygon without hole.
...
function eliminateHole(hole: Node, outerNode: Node) {
  ...
  if (outerNode) {
    ...
    // Filter collinear points around the cut.
...
// Whether sector in vertex m contains sector in vertex p in the same coordinate.
...
// Http://www.chiark.greenend.org.uk/~sgtatham/algorithms/listsort.htm
...
// The z-order of a point given coords and inverse of the longer side of data bbox.
function zOrder(x: number, y: number, minX: number, minY: number, invSize: number): number {
  // Coords are transformed into non-negative 15-bit integer rang.
