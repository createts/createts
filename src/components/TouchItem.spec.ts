...
test('should hold values', () => {
  const now = Date.now();
  const touchItem = new TouchItem(1, undefined, 100, 200, now);
...
test('should switch srcElement', () => {
  const now = Date.now();
  const srcElement = new XObject();
  const touchItem = new TouchItem(1, undefined, 100, 200, now).switchSourceElement(srcElement);
  expect(touchItem).toEqual({
    identifier: 1,
    srcElement,
    srcStageX: 100,
...
test('should clone a new object', () => {
  const touchItem = new TouchItem(1, undefined, 100, 200, Date.now());
