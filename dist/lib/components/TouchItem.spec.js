...
test('should hold values', function () {
  var now = Date.now();
  var touchItem = new _TouchItem.TouchItem(1, undefined, 100, 200, now);
...
test('should switch srcElement', function () {
  var now = Date.now();
  var srcElement = new _XObject.XObject();
  var touchItem = new _TouchItem.TouchItem(1, undefined, 100, 200, now).switchSourceElement(srcElement);
  expect(touchItem).toEqual({
    identifier: 1,
...
test('should be updated', function () {
  var now = Date.now();
  var touchItem = new _TouchItem.TouchItem(1, undefined, 100, 200, now);
  touchItem.onUpdate(new _TouchItem.TouchItem(1, undefined, 200, 200, now + 100));
  ...
  touchItem.onUpdate(new _TouchItem.TouchItem(1, undefined, 200, 300, now + 200));
  expect(touchItem.speed).toEqual(1);
  expect(touchItem.direction).toEqual(270);
  expect(touchItem.stageX).toEqual(200);
  expect(touchItem.stageY).toEqual(300);
  touchItem.onUpdate(new _TouchItem.TouchItem(1, undefined, 100, 200, now + 300));
...
test('should clone a new object', function () {
  var touchItem = new _TouchItem.TouchItem(1, undefined, 100, 200, Date.now());
